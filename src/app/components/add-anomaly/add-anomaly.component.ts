...
export class AddAnomalyComponent implements OnInit {
  ...
  createForm() {
    this.anomalyForm = this.formBuilder.group({
      title: [
        {
          value: this.anomaly.title,
          disabled: this.viewOrEdit == 'view' ? true : false,
        },
        Validators.required,
      ],
      anomalyType: [
        {
          value: this.anomaly.anomalyType,
          disabled: this.viewOrEdit == 'view' ? true : false,
        },
        Validators.required,
      ],
  ...
  saveAnomaly() {
    ...
    // this.anomalyForm.controls['title'].disable();
    // this.anomalyForm.controls['anomalyType'].disable();
    ...
    if (this.anomaly.id) {
      ...
      this.anomalyService
        .editAnomaly(this.anomaly.id, newAnomaly)
        .subscribe((anomaly) => {
