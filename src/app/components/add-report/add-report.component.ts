...
export class AddReportComponent implements OnInit {
  ...
  fillUpForm() {
    this.reportForm = this.formBuilder.group({
      ...
      latitude: [
        this.report.latitude ? this.report.latitude : '',
        Validators.required,
      ],
      longitude: [
        this.report.longitude ? this.report.longitude : '',
        Validators.required,
      ],
  ...
  getReport() {
    ...
    this.reportService.getReport(this.reportId).subscribe((res) => {
      ...
      if (this.report.anomelyReportImage) {
        this.selectedLibraryImages = this.report.anomelyReportImage.map(
          (i) => i.image
        );
      }
      this.fillUpForm();
  ...
  onUploadFiles() {
    if (this.fileToUpload == undefined) {
      ...
      return this.imageService.createImage(this.fileToUpload).subscribe(
        (response: any) => {
          this.fileUpoadInitiated = false;
          this.fileUpload = '';
  ...
  saveReport() {
    if (this.reportId !== 'add') {
      ...
      let selectedLibraryImageIds = this.selectedLibraryImages.map(
        (image) => image.id
      );
      ...
      this.loading = true;
      this.reportService
        .editReport(editedReport.id, editedReport)
        .subscribe((res) => {
          if (res) {
            this.imageService
              .createOrUpdateAnomalyReportImageWithImageIds(
                anomalyReportImageDatas
              )
              .subscribe((r) => {
                console.log(r, '::::::::::::::::');
              });
      ...
      // this.router.navigate(['/anomaly/32/report'], { relativeTo: this.route });
    }
  }
  ...
  deleteSelectedImage(image) {
    let indexToRemove = this.selectedLibraryImages.findIndex(
      (im) => im.id == image.id
    );
    ...
    files.forEach((outerFile) => {
      if (outerFile.fileEntry.name) {
        let found = this.files.find((innerFile) => {
          return (
            innerFile['fileEntry']['name'] == outerFile['fileEntry']['name']
          );
        });
        if (!found) {
  ...
  removeFile(fileName) {
    let foundIndex = this.files.findIndex(
      (element) => element['fileEntry']['name'] == fileName
    );
    if (foundIndex >= 0) {
